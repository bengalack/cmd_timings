...
Key takeaway in the image above: __THE SIZE OF THE WHITE BAR__ (and toggle settings during operation to see effects).

The screen above shows (more images [exist](project/img)):
...
* All the above is repeated for every frame.
...
|             | ⬇&nbsp;(down)   | OFF      |
| Logical operation  | 0-7 | IMP (0) (default), AND (1), OR (2), XOR (3), TIMP (4), TAND (5), TOR (6), TXOR (7) |
...
* Does screen mode affect the execution time? Affected by pixels per byte, but we need to test more (add another mode?)
* How "stable" is the execution time when paralellizing? Very unstable: Real HW shows blinking in the white area of up to 14 lines (~3200 cycles with the block size in these examples).

#### Emulators
* Are the VDP COMMAND timings perfectly emulated in emulators? NO (webMSX is too fast. And openMSX is also too fast, albeit better/closer)
* Are the VDP COMMAND + CPU timings perfectly emulated in emulators? NO (webMSX is too fast. And openMSX is also too fast, albeit better/closer)

## Possible future improvements
1. Print __current settings__ on screen for easier comparisons between emulator and real HW screenshots (LMMM vs HMMM in particular, but others too)
2. Introduce other/__flexible amounts of unrolled outis__ - now it is a constant 2048. In screen 5 the red coloring succeeds/overlaps the white coloring, and we cannot see exactly where the white ends! These two should be perfectly overlapping to be able to find the slowdown factor.
3. Introduce the choice between __OUTI, OTIR, OUT(C),A and OUT(n),A__ could give different results which could be interesting.
4. Introduce some __"delay" factors__ to see how they affect the slowdown. Ref the VDP timing doc, the CPU and the VDP is fighting for "slots", and how this timing is matching is quite coincidental. Start time of every command matters, and by skewing these, we might see different results.
5. Maybe: Include other __screen modes__, like screen 6 and 7 (but I cannot see why these should bring new info...).
